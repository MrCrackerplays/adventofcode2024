...
(function main () {
	...
	let search = function(trail_ends, visited, current_coords, to_match) {
		...
		if (value === 9) {
			if (inside(trail_ends, current_coords)) return 0;
			trail_ends.push(current_coords);
			return 1;
		}
		visited.push(current_coords);
		return (search(trail_ends, visited, [y + 1, x], to_match + 1)
			+ search(trail_ends, visited, [y - 1, x], to_match + 1)
			+ search(trail_ends, visited, [y, x + 1], to_match + 1)
			+ search(trail_ends, visited, [y, x - 1], to_match + 1));
	}
